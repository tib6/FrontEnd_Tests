namespace FrontEnd_Tests.Tests
{
    ...
    public class GoogleTest : BaseFixture
    { 
        private readonly string search = "alol";

        [Test]
        public void GoogleTests_Search()
        {
            ...
            ExtentTestManager.GetTest().Log(Status.Info, "Get title succesfully!");
            googlePage.quit();
        }

        [Test]
        public void NewTabNavigation()
        {
            var googlePage = new GooglePage(Driver);
            var googleSecondPage = new GoogleSecondPage(Driver);
